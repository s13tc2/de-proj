...
def _download_taxi_data():
    ...
    for filename in [f["name"] for f in files]:
        ...
        try:
            s3_hook.load_string(
                string_data=response.text, key=s3_key, bucket_name=BUCKET_NAME
            )
...
def transform_citi_bike_data(df):
    ...
    # ).to_crs("EPSG:4326")
    taxi_zones = geopandas.read_file('https://d37ci6vzurychx.cloudfront.net/misc/taxi_zones.zip').to_crs('EPSG:4326')
...
taxi_to_db = S3PandasToPostgres(
    task_id="taxi_to_db",
    dag=dag,
    minio_conn_id="my_aws_conn",
    ...
    pre_read_transform=lambda x: io.BytesIO(x),
)

citi_bike_to_db = S3PandasToPostgres(
